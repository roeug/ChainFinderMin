...
function T_DataModuleOOB.ReadSpecialCaptureTotal(const aThZpA_s: integer; var aVarSigma, aVarRI: double): Boolean;
var
...
function T_DataModuleOOB.ReadSpecialCaptureDetail(const aParentThZpA_s, aChildThZpA_s: integer;
 var aVarSigma, aVarG_factor, aVarRI: double): Boolean;
var
 FoundCount, FirstNo, I: integer;
begin
 Result:= False;
...
constructor T_DataModuleOOB.Create;
begin
 inherited;
 fLoadInProgress:= False;
 PanelDatabaseName:= nil;
 CreateDataCollections(nil);
end;

function T_DataModuleOOB.GetSigmaFastFissionForThZpA_s(
 const aThZpA_s: integer): double;
var
 Founded: integer;
begin
 if (fVFastFissions = nil) then
